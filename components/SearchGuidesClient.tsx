...
import { CloudDownload, Filter, RefreshCw, SlidersHorizontal } from "lucide-react";
...
export function SearchGuidesClient({
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[320px_minmax(0,1fr)]">
      <section className="h-fit rounded-lg border border-slate-200 bg-white p-4 shadow-sm xl:sticky xl:top-24">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {quickTerms.map((term) => (
            <button
              ...
              className="inline-flex min-h-8 items-center justify-center rounded-md border border-slate-300 bg-white px-2.5 py-1 text-xs font-medium text-slate-700 transition hover:border-emerald-300 hover:bg-emerald-50"
            >
              {term}
            </button>
          ))}
        ...
        <button
          ...
          className="mt-4 inline-flex min-h-9 w-full items-center justify-center rounded-md border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:border-emerald-300 hover:bg-emerald-50"
        >
          Limpiar filtros
        </button>
      </section>
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-3">
            ...
            {!overrideGuides ? (
              <button
                type="button"
                onClick={() =>
                  void fetchRemoteGuides(debouncedQuery, category, sourceType)
                }
                className="inline-flex min-h-8 items-center gap-2 rounded-md border border-slate-300 bg-white px-2.5 py-1 text-xs font-semibold text-slate-700 transition hover:border-emerald-300 hover:bg-emerald-50"
              >
